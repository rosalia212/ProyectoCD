...
# Script de instalación de paquetes necesarios para el proyecto
...
cargar_datos <- function(ruta_relativa) {
  ruta <- tryCatch(
    normalizePath(ruta_relativa, mustWork = TRUE),
    error = function(e) {
      stop("No se pudo encontrar el archivo: ", ruta_relativa)
    }
  )
  read_csv(ruta, locale = locale(encoding = "UTF-8"))
}
