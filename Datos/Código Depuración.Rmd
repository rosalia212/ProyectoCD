...
esperanzadevida <- read_excel("esperanza_de_vida_mujer.xlsx")
ProductoGDP <- read_excel("gdp_pcap.xlsx")
nacimientos <- read_excel("nacimientos_por_mujer.xlsx")
...
```
# Modificando errores en los países
```{r}
...
# Seleccionando los países y creando la base de datos completa
```{r}
...
# Inicio del Análisis para el reto 1
```{r}
#Resumen estadístico general de cada variable
...
```{r}
# Asegurar que el campo "year" sea numérico
base_completa$year <- as.numeric(base_completa$year)

# 1. País que más veces ha tenido la mayor esperanza de vida
...
# Mostrar el país con más "primeros lugares" en esperanza de vida
head(pais_mas_lider, 1)

# Ver los principales 5 líderes históricos
...
# Realizando gráficos generales
```{r}
ggplot(resumen_por_ano, aes(x = year, y = promedio)) +
  geom_line(color = "steelblue", size = 1) +
  labs(title = "Promedio mundial de esperanza de vida femenina por año",
       x = "Año",
...
ggplot(base_completa, aes(x = nacimientos_por_mujer)) +
  ...
  labs(title = "Distribución de nacimientos por mujer", x = "Hijos por mujer", y = "Frecuencia") +
  theme_minimal()
#Esto me dice que la mayoría de las mujeres durante los años tienen aproximadamente 1
```
```{r}
names(base_completa)
...
ggplot(base_completa, aes(x = esperanza_de_vida)) +
  ...
  labs(title = "Distribución de la esperanza de vida femenina",
       x= "Esperanza de vida (años)",
       y="Frecuencia") +
  theme_minimal()
```
#Reto 1 solicitaba Saber si tenía Outliers
...
#Agregué manualmente Kosovo porque no lo reconoce la libreria countrycoud
...
ggplot(base_completa, aes(x = gdp_bajo, y = esperanza_de_vida)) +
  ...
  labs(title = "Esperanza de vida según nivel de GDP", y = "Esperanza de vida", x = "Grupo de GDP") +
  theme_minimal()
```
...
ggplot(base_completa, aes(x = gdp_grupo, y = esperanza_de_vida, fill = gdp_grupo)) +
  geom_boxplot() +
  labs(title = "Esperanza de vida según nivel de GDP",
       x = "Grupo de GDP",
       y = "Esperanza de vida femenina (años)") +
...
base_completa %>%
  ...
  labs(title = "Países con alta fecundidad (> 5 hijos)", x = "Año", y = "Hijos por mujer") +
  theme_minimal()
```
...
base_completa %>%
  ...
  labs(title = "Países con alta esperanza de vida (> 80 años)", x = "Año", y = "Esperanza de vida") +
  theme_minimal()
...
base_completa %>%
  ...
  labs(title = "Países con alta esperanza de vida(>80) por continente",
       x = "Año",
       y = "Esperanza de vida",
       color = "Continente") +
  theme_minimal()
...
ggplot(base_completa, aes(x = producto_por_capita, y = esperanza_de_vida)) +
  ...
  labs(title = "Relación entre GDP per cápita y esperanza de vida", x = "GDP per cápita (log)", y = "Esperanza de vida") +
  theme_minimal()
```
...
ggplot(base_completa, aes(x = nacimientos_por_mujer, y = esperanza_de_vida)) +
  ...
  labs(title = "Relación entre hijos por mujer y esperanza de vida", x = "Hijos por mujer", y = "Esperanza de vida") +
  theme_minimal()
```
...
ggplot(base_completa, aes(x = producto_por_capita, y = nacimientos_por_mujer)) +
  ...
  scale_x_log10() +
  geom_smooth(method = "loess", se = FALSE, color = "black") +
  labs(title = "Relación entre GDP y fecundidad", x = "GDP per cápita (log)", y = "Hijos por mujer") +
  theme_minimal()
```
```{r}
base_completa %>%
  filter(nacimientos_por_mujer > 5) %>%
  distinct(country)  # te da solo los países únicos
...
base_completa %>%
  ...
  geom_point(alpha = 0.6) +
  labs(title = "Países con alta fecundidad (>5 hijos por mujer) por continente",
       x = "Año",
       y = "Hijos por mujer",
       color = "Continente") +
  theme_minimal()
...
ggplot(brasil_data, aes(x = gdp_grupo, y = esperanza_de_vida, fill = gdp_grupo)) +
  geom_boxplot() +
  labs(title = "Esperanza de vida en Brasil según nivel de GDP",
       x = "Grupo de GDP",
       y = "Esperanza de vida femenina (años)") +
  scale_fill_manual(values = c("Bajo GDP" = "red", "Alto GDP" = "steelblue")) +
  theme_minimal()
...
```{r}
# Filtrar datos solo del año 2021
...
ggplot(datos_2021, aes(x = gdp_grupo, y = esperanza_de_vida, fill = gdp_grupo)) +
  geom_boxplot() +
  labs(title = "Esperanza de vida femenina en 2021 según nivel de GDP",
       x = "Grupo de GDP",
       y = "Esperanza de vida femenina (años)") +
  scale_fill_manual(values = c("Bajo GDP" = "red", "Alto GDP" = "steelblue")) +
  theme_minimal()
...
```{r}
# Filtrar solo datos de Brasil en el año 2021
...
# O crear un gráfico de punto único
...
```
```{r}
# Filtrar solo datos del año 2021 y no NA
...
ggplot(base_2021, aes(x = continente, y = esperanza_de_vida, fill = continente)) +
  geom_boxplot() +
  labs(title = "Distribución de la esperanza de vida femenina por continente (2021)",
       x = "Continente",
       y = "Esperanza de vida femenina (años)") +
  theme_minimal() +
  theme(legend.position = "none")
...
```{r}
# Filtrar datos del año 2021 y eliminar valores faltantes
base_2021 <- base_completa %>%
  filter(year == 2021) %>%
  drop_na(esperanza_de_vida, nacimientos_por_mujer)

# Escalar ambas variables (para compararlas en el mismo gráfico)
...
# Seleccionar los países con esperanza de vida más alta (o puedes cambiar a los más bajos)
...
ggplot(base_barras, aes(x = reorder(country, -valor), y = valor, fill = variable)) +
  ...
  labs(
    title = "Comparación estandarizada entre esperanza de vida y fecundidad (Top 10 países)",
    x = "País",
...
```{r}
# Filtrar y limpiar datos del año 2021
base_2021 <- base_completa %>%
  filter(year == 2021) %>%
...
ggplot(base_continente, aes(x = continente, y = valor, fill = variable)) +
  geom_col(position = "dodge") +
  labs(
    title = "Comparación estandarizada de esperanza de vida y fecundidad por continente (2021)",
    x = "Continente",
    y = "Valor estandarizado",
    fill = "Variable"
  ) +
